...
export default function App() {
  ...
  const startGuided = () => { setQuick(false); setEditing(false); setStep(1) }

  const run = (p: Profile, t: Tag[], withKpass: boolean) => {
  ...
  return (
    <main className="app">
      ...
      {step === 4 && query && <Results query={query} results={results} relaxed={relaxed} mileResults={mileResults} onEdit={edit} today={today} quick={quick} />}
    </main>
  )
}
